...
Options:
    ...
    --fedimint-cli        Path to fedimint-cli binary (default: fedimint-cli)
    --data-dir            Client data directory (default: ~/.escrow-recovery/)
    --action              claim-timeout (default) or info

What this does:
    1. Joins the Fedimint federation using the invite code
...
Requirements:
    - fedimint-cli binary (with the escrow module compiled in)
...
def run_cli(cli_path: str, data_dir: str, *args) -> str:
    """Run fedimint-cli command and return stdout."""
    cmd = [cli_path, "--data-dir", data_dir] + list(args)
    print(f"  > {' '.join(cmd[:6])}...")
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
    if result.returncode != 0:
        print(f"  ERROR: {result.stderr.strip()}")
        raise RuntimeError(f"fedimint-cli failed: {result.stderr.strip()}")
...
def main():
    ...
    parser.add_argument("--escrow-id", help="Fedimint escrow ID")
    ...
    parser.add_argument("--fedimint-cli", default="fedimint-cli",
                        help="Path to fedimint-cli binary (default: fedimint-cli)")
