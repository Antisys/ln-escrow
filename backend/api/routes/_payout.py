...
Fedimint-only: execute_fedimint_payout() — claim escrow → pay via LN.
"""
import asyncio
import logging
import os
...
async def resolve_to_routeable_invoice(
    ...
    No LND route pre-check: payouts go through the Fedimint gateway which has
...
# ============================================================================
# execute_fedimint_payout() — the core resolution function
# ============================================================================

async def execute_fedimint_payout(
    deal: dict,
    payout_type: str,
    ...
    """
    Execute a Fedimint-based payout: claim escrow → pay recipient via LN.
    ...
    if not escrow_id:
        raise ValueError(f"Deal {deal_id} has no fedimint_escrow_id")
    ...
    if '@' in invoice and not invoice.lower().startswith('ln'):
        ...
        comment = f"trustMeBro payout: {deal_title}" if deal_title else f"trustMeBro payout ({deal['price_sats']} sats)"
        try:
    ...
    async with inflight():
        ...
        t_fedimint_start = time.monotonic()
        try:
            # NON-CUSTODIAL: deals with buyer_escrow_pubkey have user keys registered
            # in the Fedimint escrow. The service CANNOT use its own key — it MUST use
            ...
            if is_refund or timeout_claim:
                ...
                if timeout_sig:
                    await fedimint.refund_deal_delegated(escrow_id, timeout_sig, invoice)
                elif has_user_keys:
                    ...
                    await fedimint.refund_deal_escrow(escrow_id, invoice)
            else:
                # Non-custodial release: buyer's Schnorr signature required
                if buyer_escrow_signature:
                    await fedimint.release_deal_delegated(
                    ...
                    await fedimint.release_deal_escrow(escrow_id, secret_code, invoice)
        except Exception as payout_err:
            t_fedimint_done = time.monotonic()
            logger.error(
                "Fedimint payout failed for deal %s after %.1fs: %s",
                deal_id, t_fedimint_done - t_fedimint_start, payout_err,
            )
            ...
            if escrow_claimed and is_timeout:
                ...
                # claim-timeout-delegated-and-pay are ATOMIC Fedimint operations
                ...
                logger.error(
                    ...
                    "e-cash stuck in wallet. Admin must resolve manually via fedimint-cli.",
                    deal_id, escrow_id,
                )
        ...
        t_fedimint_done = time.monotonic()
        logger.info(
            "[TIMING] Deal %s: Fedimint claim+pay took %.1fs (total so far: %.1fs)",
            deal_id[:8], t_fedimint_done - t_fedimint_start, t_fedimint_done - t_start,
        )
        ...
        audit_log.info("fedimint_payout_success",
...
async def execute_oracle_payout(
    ...
    Unlike execute_fedimint_payout() (which claims an escrow), this function handles
    ...
    async with inflight():
        ...
        try:
            await fedimint.resolve_and_pay_via_oracle(escrow_id, attestations, invoice)
        except Exception as payout_err:
