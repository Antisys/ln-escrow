"""
Release endpoint: buyer triggers release → Fedimint escrow release → LN payout to seller.
"""
import asyncio
import hashlib
import logging
...
async def release_deal(deal_id: str, body: ReleaseDealRequest):
    """Buyer releases funds to seller. Claims Fedimint escrow, pays seller via LN."""
    ...
    if deal['status'] == DealStatus.ACTIVE.value and deal.get('fedimint_escrow_id') and deal.get('funded_at'):
        logger.warning(
            "Deal %s in 'active' state but has escrow_id %s and funded_at — auto-advancing to funded",
            deal_id, deal['fedimint_escrow_id']
        )
    ...
    if not deal.get('seller_payout_invoice'):
        raise HTTPException(status_code=400, detail="Seller has not submitted a payout invoice yet")
    ...
    if not stored_hash:
        ...
        logger.error("Deal %s has no fedimint_secret_code_hash — cannot verify release", deal_id)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # execute_fedimint_payout handles its own status transitions (releasing → completed).
