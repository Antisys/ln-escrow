...
async def get_signing_status(deal_id: str):
    """Get the current signing phase status for a deal (Fedimint mode)."""
    ...
    # Fedimint mode: funded only when deal status is funded/shipped/beyond
    ...
    fedimint_funded = deal.get('status') in funded_statuses and bool(deal.get('fedimint_escrow_id'))
    return SigningStatusResponse(
        deal_id=deal_id,
        phase="funded" if fedimint_funded else (
        ...
        ready_for_resolution=fedimint_funded,
        # Use escrow_id as funding_txid so the frontend knows when the deal is funded
        funding_txid=deal.get('fedimint_escrow_id') if fedimint_funded else None,
    )
