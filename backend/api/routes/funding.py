...
Fedimint path: buyer pays LN invoice → e-cash arrives in service's federation wallet
→ service locks it into a Fedimint escrow → deal becomes funded.

Invoice creation: fedimint-cli module ln invoice <msats>
Payment detection: fedimint-cli await-invoice <operation_id> (with timeout, non-blocking)
"""
import asyncio
import logging
import re
...
class CreateInvoiceBody(BaseModel):
    ...
    # NON-CUSTODIAL: buyer_pubkey is REQUIRED. Without it, the Fedimint escrow uses the
...
async def _create_lightning_invoice_inner(deal_id: str, body: CreateInvoiceBody):
    ...
    # seller_pubkey in the Fedimint escrow (non-custodial requirement).
    ...
    try:
        ...
        existing_escrow = deal.get('fedimint_escrow_id')
        existing_op_id = deal.get('ln_operation_id')
        if existing_escrow and existing_op_id:
            already_paid = await fedimint.check_funding_invoice_paid(existing_op_id)
            if already_paid:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            if existing_bolt11:
                ...
                final_check = await fedimint.check_funding_invoice_paid(existing_op_id)
                if final_check:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="Invoice already paid. Refresh the page to see updated status."
        ...
        # When buyer pays, await_receive_into_escrow atomically creates the Fedimint escrow
        ...
        # in the Fedimint escrow. The seller's own key — not the service's key.
        ...
        ln_result = await fedimint.create_funding_invoice_with_escrow(
        ...
        update_fields = dict(
            ...
            fedimint_secret_code_hash=body.secret_code_hash,
            ...
            fedimint_timeout_block=timeout_block,
        )
...
async def check_lightning_invoice(deal_id: str):
    ...
    try:
        ...
        if escrow_id:
            ...
            result = await fedimint.check_funding_invoice_with_escrow(
                operation_id=operation_id,
                escrow_id=escrow_id,
                amount_sats=deal.get('invoice_amount_sats', deal['price_sats']),
                secret_code_hash=deal_storage.get_secret_code_hash(deal_id) or '',
                timeout_block=deal.get('fedimint_timeout_block', 0),
                timeout_action=deal.get('timeout_action', 'refund'),
            ...
            # Legacy path removed: deals without fedimint_escrow_id should not exist.
            ...
            logger.error("check-ln-invoice: deal %s has no fedimint_escrow_id — legacy path removed", deal_id)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        # Transient CLI timeout (fedimintd DB contention) → return awaiting, not 500.
        # The frontend/test polls again in a few seconds and will get the real status.
        from backend.fedimint.escrow_client import EscrowClientError
        if isinstance(e, EscrowClientError) and "timed out" in str(e):
            logger.warning("check-ln-invoice: transient fedimint-cli timeout for deal %s — returning awaiting", deal_id)
            return InvoiceStatusResponse(deal_id=deal_id, payment_hash=payment_hash, paid=False)
