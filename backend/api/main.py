"""
trustMeBro API — Decentralized escrow service using Lightning Network and Fedimint.
"""
import asyncio
import logging
import os
...
from backend.api.routes import health, deals, auth, qr, websockets
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    app.state.escrow_service = True  # Fedimint mode — placeholder for health check
    ...
    # Start Fedimint oracle listener
    try:
        ...
        logger.info("Fedimint oracle listener started (%d relays)", len(oracle_listener.relays))
    except Exception as e:
        logger.error("Fedimint oracle listener failed to start: %s", e)
...
app = FastAPI(
    title="trustMeBro API",
    description="Decentralized escrow service using Lightning Network and Fedimint",
    version=VERSION,
    lifespan=lifespan
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=os.getenv('CORS_ORIGINS', 'https://trustbro.trade').split(','),
