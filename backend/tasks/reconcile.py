...
In Fedimint mode there is no chain to check — we simply log and leave the deals
...
def reconcile_stuck_deals():
    ...
    In Fedimint mode we cannot check chain state, so we leave these deals as-is.
    The process_stalled_payouts() loop in timeout_handler will retry the LN payout
    if the Fedimint escrow was already claimed (release_txid/refund_txid set).
    """
