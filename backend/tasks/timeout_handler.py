...
1. process_expired_deals() — find newly expired deals, execute Fedimint timeout + LN payout
2. process_stalled_payouts() — retry failed LN payouts (Fedimint escrow already claimed)
"""
import asyncio
import logging
import re
...
# Regex to extract block heights from Fedimint timelock error
...
def _parse_timelock_error(error_str: str) -> tuple[int, int] | None:
    """Extract (current_block, required_block) from a Fedimint timelock error, or None."""
...
async def process_expired_deals(vault_service=None):
    """
    Find expired deals and execute their timeout_action via Fedimint.
    - 'refund': claim Fedimint escrow → pay buyer via LN
    - 'release': claim Fedimint escrow → pay seller via LN
    """
    ...
    for deal in expired:
        ...
        if not deal.get('fedimint_escrow_id'):
            logger.warning("[Timeout] Deal %s expired but has no Fedimint escrow, marking expired", deal_id[:8])
...
async def _execute_timeout_action(deal, payout_type: str):
    """Execute timeout action (release or refund): claim Fedimint escrow → pay via LN."""
    deal_id = deal['deal_id']
    ...
    if not invoice:
        ...
        # because the Fedimint escrow's claim-timeout-delegated requires the seller's
    ...
    logger.info("[Timeout] Deal %s executing Fedimint %s payout", deal_id[:8], payout_type)
    try:
...
async def process_stalled_payouts():
    ...
    Covers deals where the Fedimint timeout block wasn't reached yet when the
    ...
    for deal in candidates:
        ...
        # This covers deals where the Fedimint timeout block wasn't reached yet
        # when the initial timeout handler ran, or where claim-timeout-and-pay
        # failed for transient reasons (DB lock, network, etc).
        if deal.get('status') != 'expired' or not deal.get('fedimint_escrow_id'):
            continue
