...
def _make_deal(**overrides):
    ...
    deal = {
        ...
        "fedimint_escrow_id": ESCROW_ID,
        "fedimint_secret_code_hash": SECRET_CODE_HASH,
        "invoice_bolt11": None,
        "ln_invoice": None,
...
def app_and_mocks():
    ...
    (Fedimint, LND, Lightning Address resolution) are mocked.
    """
    deal_store = {}
    ...
    # Mock FedimintEscrowService
    fedimint_mock = MagicMock()
    fedimint_mock.release_deal_escrow = AsyncMock(return_value=None)
    fedimint_mock.refund_deal_escrow = AsyncMock(return_value=None)
    fedimint_mock.resolve_deal_escrow_via_oracle = AsyncMock(return_value=None)
    fedimint_mock.client = MagicMock()
    fedimint_mock.client.claim_escrow = AsyncMock(return_value=None)
    fedimint_mock.client.claim_timeout = AsyncMock(return_value=None)
    fedimint_class = MagicMock(return_value=fedimint_mock)
    ...
    patches = [
        ...
        patch("backend.fedimint.fedimint_service.FedimintEscrowService", fedimint_class),
...
class TestSecretCodeNonCustodial:
    ...
    def test_release_requires_secret_code(self, app_and_mocks):
        ...
        # Fedimint must NOT have been called
        m["fedimint_mock"].release_deal_escrow.assert_not_called()
        m["fedimint_mock"].client.claim_escrow.assert_not_called()
    ...
    def test_hash_verification_is_sha256(self, app_and_mocks):
        ...
        m["deal_store"][DEAL_ID] = _make_deal(
            status="funded",
            fedimint_secret_code_hash=custom_hash,
        )
...
#
# If Fedimint claim or LN payment fails, the deal must revert to its
...
class TestFailedPayoutRevert:
    ...
    def test_release_failure_reverts_to_funded(self, app_and_mocks):
        """Fedimint claim-and-pay fails → status reverts to 'funded', not stuck on 'releasing'."""
        m = app_and_mocks
        m["fedimint_mock"].release_deal_escrow.side_effect = Exception("LN route not found")
        m["fedimint_mock"].client.claim_escrow.side_effect = Exception("LN route not found")
        m["deal_store"][DEAL_ID] = _make_deal(status="funded")
    ...
    def test_refund_failure_reverts_to_funded(self, app_and_mocks):
        """Fedimint claim-timeout fails → status reverts to 'funded', not stuck on 'refunding'."""
        m = app_and_mocks
        m["fedimint_mock"].refund_deal_escrow.side_effect = Exception("timeout not reached")
        m["fedimint_mock"].client.claim_timeout.side_effect = Exception("timeout not reached")
        m["deal_store"][DEAL_ID] = _make_deal(status="funded")
    ...
    def test_release_failure_from_shipped_reverts_to_shipped(self, app_and_mocks):
        ...
        m = app_and_mocks
        m["fedimint_mock"].release_deal_escrow.side_effect = Exception("network timeout")
        m["fedimint_mock"].client.claim_escrow.side_effect = Exception("network timeout")
...
# must return success WITHOUT calling Fedimint again.
# ===========================================================================
...
class TestIdempotency:
    ...
    def test_release_idempotent_when_paid(self, app_and_mocks):
        """Release on already-paid deal → 200, Fedimint NOT called."""
        m = app_and_mocks
        m["deal_store"][DEAL_ID] = _make_deal(
            status="completed",
            payout_status="paid",
    ...
    def test_refund_idempotent_when_paid(self, app_and_mocks):
        """Refund on already-paid deal → 200, Fedimint NOT called."""
        m = app_and_mocks
        m["deal_store"][DEAL_ID] = _make_deal(
            status="refunded",
            buyer_payout_status="paid",
        ...
        assert resp.status_code == 200
        m["fedimint_mock"].refund_deal_escrow.assert_not_called()
        m["fedimint_mock"].client.claim_timeout.assert_not_called()
...
#
# If fedimint_escrow_id is missing, no payout operation should be attempted.
# ===========================================================================
...
class TestNoEscrowNoPayout:
    ...
    def test_release_blocked_without_escrow(self, app_and_mocks):
        """Release with no fedimint_escrow_id → 400."""
        m = app_and_mocks
        m["deal_store"][DEAL_ID] = _make_deal(
            status="funded",
            fedimint_escrow_id=None,
    ...
    def test_refund_blocked_without_escrow(self, app_and_mocks):
        """Refund with no fedimint_escrow_id → 400."""
        m = app_and_mocks
        m["deal_store"][DEAL_ID] = _make_deal(
            status="funded",
            fedimint_escrow_id=None,
    ...
    def test_admin_blocked_without_escrow(self, app_and_mocks):
        """Admin resolve with no fedimint_escrow_id → 400."""
        m = app_and_mocks
        m["deal_store"][DEAL_ID] = _make_deal(
            status="disputed",
            fedimint_escrow_id=None,
...
class TestPostClaimConsistency:
    ...
    def test_failed_payout_no_txid_no_completed_at(self, app_and_mocks):
        ...
        m = app_and_mocks
        m["fedimint_mock"].release_deal_escrow.side_effect = Exception("boom")
        m["fedimint_mock"].client.claim_escrow.side_effect = Exception("boom")
        m["deal_store"][DEAL_ID] = _make_deal(status="funded")
