...
def test_execute_fedimint_payout_exists():
    """execute_fedimint_payout must exist (Fedimint-only resolution path)."""
    from backend.api.routes._payout import execute_fedimint_payout
    assert callable(execute_fedimint_payout)
...
def test_deal_storage_functions_extended():
    """Fedimint-related deal storage functions must exist."""
    from backend.database import deal_storage
