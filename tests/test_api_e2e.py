...
SERVER_FEDIMINT_CLI = os.environ.get("SERVER_FEDIMINT_CLI", "fedimint-cli")
...
timeout {timeout} {SERVER_FEDIMINT_CLI} --data-dir $FM_CLIENT_DIR module ln pay '{bolt11}' 2>&1
""",
    ]
...
def api(method: str, path: str, _retry_on_429: bool = True, **kwargs) -> requests.Response:
    ...
    url = f"{BASE_URL}{path}"
    # fedimint-cli calls via SSH can take 30+ seconds; use 90s default
...
class TestNonCustodialGuards:
    ...
    def test_deal_db_secret_code_is_null(self, funded_deal):
        """The deal's fedimint_secret_code should be null in DB after delivery."""
        deal_id = funded_deal[0]
        ...
        assert deal_resp.json().get("fedimint_secret_code") is None
...
class TestHappyPath:
    ...
    def test_create_invoice_requires_funded_deal(self):
        ...
        assert deal_state["fedimint_escrow_id"] is not None
        assert deal_state.get("fedimint_secret_code") is None  # never stored
