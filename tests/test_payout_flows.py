...
All escrow operations go through FedimintEscrowService.

Mocks at three layers:
1. Signature verification → always passes
2. Deal storage → in-memory dicts
3. External services → FedimintEscrowService (release_deal_escrow, refund_deal_escrow), resolve_lightning_address, WebSocket, notifications
"""
import hashlib
import os
import time
...
def app_and_mocks():
    ...
    - FedimintEscrowService (claim + pay_ln_invoice)
    ...
    fedimint_mock.resolve_and_pay_via_oracle = AsyncMock(return_value={"status": "success"})
    fedimint_mock.pay_from_wallet = AsyncMock(return_value={"status": "success"})
    ...
    fedimint_mock.get_escrow_info = AsyncMock(return_value=_mock_escrow_info)
...
class TestNormalRelease:
    ...
    def test_release_all_ready(self, app_and_mocks):
        ...
        m["fedimint_mock"].release_deal_escrow.assert_called_once()
...
class TestTimeoutRelease:
    ...
    async def test_timeout_release_no_escrow(self, app_and_mocks):
        """#15: No fedimint_escrow_id → expired (parked)."""
        m = app_and_mocks
        deal = _make_deal(
            status="funded",
            timeout_action="release",
...
class TestStatusGuards:
    ...
    def test_admin_rejects_no_escrow(self, app_and_mocks):
        """Admin cannot resolve a deal with no Fedimint escrow."""
        m = app_and_mocks
        m["deal_store"][DEAL_ID] = _make_deal(
            status="disputed",
            fedimint_escrow_id=None,
...
class TestMissingEscrow:
    """Endpoints must fail gracefully when Fedimint escrow is missing."""
